...
class NetworkFeatures:
    ...
    def load_network(self, network_filename: str, total_edges: int):
        ...
        if network_filename.endswith('.gz'):
            network_dataset = pd.read_csv(network_filename, compression='gzip')
        else:
            network_dataset = pd.read_csv(network_filename)

        numpy_df = network_dataset.to_numpy()
        graph = from_edge_list(numpy_df, directed=True)
        return graph
    ...
    def calculate_page_rank(self, graph, damping_factor=0.85, iterations=100, weights=None) -> list[float]:
        ...
        if weights is not None:
            ...
            scores = pagerank.fit_transform(graph.adjacency, weights)
        else:
            pagerank = PageRank(damping_factor=damping_factor, n_iter=iterations)
    ...
    def calculate_hits(self, graph) -> tuple[list[float], list[float]]:
        ...
        hits = HITS()
        hits = hits.fit(graph.adjacency)
        hub_scores, authority_scores = hits.scores_row_, hits.scores_col_
